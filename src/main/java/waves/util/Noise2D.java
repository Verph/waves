...
public interface Noise2D
{
    ...
    default Noise2D pow(double power)
    {
        return (x, y) -> Math.pow(Noise2D.this.noise(x, y), power);
    }

    default Noise2D easeInOutExpo(double min, double max)
    {
        return (x, y) -> WaveHelpers.easeInOutExpo(Noise2D.this.noise(x, y), min, max);
    }

    default Noise2D easeInOutExpoNorm(double min, double max, double lowerBound, double upperBound)
    {
        return (x, y) -> WaveHelpers.easeInOutExpoNorm(Noise2D.this.noise(x, y), min, max, lowerBound, upperBound);
    }

    default Noise2D easeOutInExpo(double min, double max)
    {
        return (x, y) -> WaveHelpers.easeOutInExpo(Noise2D.this.noise(x, y), min, max);
    }

    default Noise2D easeInExpo(double min, double max)
    {
        return (x, y) -> WaveHelpers.easeInExpo(Noise2D.this.noise(x, y), min, max);
    }

    default Noise2D easeOutExpo(double min, double max)
    {
        return (x, y) -> WaveHelpers.easeOutExpo(Noise2D.this.noise(x, y), min, max);
    }
    ...
    default Noise2D lazyProductPow(Noise2D other)
    {
        return (x, y) -> {
            final double value = Noise2D.this.noise(x, y);
            return value == 0 ? 0 : Math.pow(value, other.noise(x, y));
        };
    }
