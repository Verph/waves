...
public class WaveParticle extends TextureSheetParticle
{
    ...
    public void render(VertexConsumer buffer, Camera camera, float partialTicks)
    {
        ...
        if (WaveHelpers.isWithinAngle(player.getLookAngle(), wavePos, minecraft.options.fov().get() + 5) && player.getEyePosition().distanceTo(wavePos) <= minecraft.options.renderDistance().get().doubleValue() * 16.0D)
        {
            ...
            builder.vertex(matrix, avector3[0].x(), avector3[0].y(), avector3[0].z()).color(r, g, b, a).uv(u1, v1).uv2(j).endVertex();
            builder.vertex(matrix, avector3[1].x(), avector3[1].y(), avector3[1].z()).color(r, g, b, a).uv(u1, v0).uv2(j).endVertex();
            builder.vertex(matrix, avector3[2].x(), avector3[2].y(), avector3[2].z()).color(r, g, b, a).uv(u0, v0).uv2(j).endVertex();
            builder.vertex(matrix, avector3[3].x(), avector3[3].y(), avector3[3].z()).color(r, g, b, a).uv(u0, v1).uv2(j).endVertex();
